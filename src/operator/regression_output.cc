...
Operator *CreateRegressionOutputOp<cpu>(reg_enum::RegressionOutputType type,
  ...
  switch (type) {
    ...
    case reg_enum::kAngle:
        return new RegressionOutputOp<cpu, mshadow_op::identity, mshadow_op::sin_minus>(param);
    default:
...
MXNET_REGISTER_OP_PROPERTY(AngleRegressionOutput, RegressionOutputProp<reg_enum::kAngle>)
.describe("Use Angle regression for final output, this is used on final output of a net.\n"
          "For prediction of orientation angle.")
.add_argument("data", "Symbol", "Input data to function.")
.add_argument("label", "Symbol", "Input label to function.")
.add_arguments(RegressionOutputParam::__FIELDS__());
